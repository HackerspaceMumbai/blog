...
import Auth0ApplicationProperties from './auth0_application_properties.png';
import Auth0OauthApplicationUrl from './auth0_oauth_application_url.png';
import AzurePortalHostingPlan from './azure__portal_hosting_plan.png';
import AzureswaApplicationSecretsPortal from './azureswa_application_secrets_portal.png';
import SwacliEmulatorCustomauth from './swacli_emulator_customauth.png';
import WeatherforecastCustomauthn from './weatherforecast_customauthn.png';
import WeatherforecastCustomauthzFail from './weatherforecast_customauthz_fail.png';
import Auth0AddRole from './auth0_add_role.png';
import Auth0AddUserRole from './auth0_add_user_role.png';
import Auth0CustomAction from './Auth0_custom_action.png';
import Auth0AddAction from './auth0_add_action.png';
import Auth0CustomActionCode from './auth0_custom_action_code.png';
import Auth0ChooseFlow from './auth0_choose_flow.png';
import Auth0FlowAddAction from './auth0_flow_add_action.png';
import Auth0FlowApplyAction from './auth0_flow_apply_action.png';
import Auth0SampleAppProfile from './auth0_sample_app_profile.png';
import VscodeAzureSWAAzureFunction from './vscode_AzureSWA_AzureFunction.png';
import AzurefunctionApiRoleMgmt from './azurefunction_api_role_mgmt.png';
import AzureswaApplInsight from './azureswa_appl_insight.png';
import WeatherforecastCustomauthzSuccess from './weatherforecast_customauthz_success.png';
import VscodeAzureSWAExtension from './vscode_AzureSWA_extension.png';
import VscodeAzureSWAExtensionInitial from './vscode_AzureSWA_extension_initial.png';
import VscodeAzureSWAMarketplace from './vscode_AzureSWA_marketplace.png';
import MsdocsAzureswaApiConstraints from './msdocs_azureswa_api_constraints.png';
import VscodeExtensionGithubAction from './vscode_extension_github_action.png';
import VscodeExtensionCommandPalletteCreateApi from './vscode_extension_command_pallette_create_api.png';
import VscodeApiFileExplorer from './vscode_api_file_explorer.png';
import SwaCliApiOptionWindows from './swa_cli_api_option_windows.png';
import MsdocsAzureswaApiCreation from './msdocs_azureswa_api_creation.png';
import WeatherforecastApiNomenclatureFail from './weatherforecast_api_nomenclature_fail.png';
import GithubActionLogAzureswaApiWarning from './github_action_log_azureswa_api_warning.png';
import GithubActionLogMissingApiWarning from './github_action_log_missing_api _warning.png';
...
<Image src={Auth0ApplicationProperties} alt="The Auth0 Application Details page with the Client ID and Client Secret information redacted" />
...
<Image src={Auth0OauthApplicationUrl} alt="Auth0 Application Details page with the Application URIs " />
...
1. Visit the Hosting Plan tab under the Static Web App in the Azure Portal to convert the plan from Free to Standard.
   <Image src={AzurePortalHostingPlan} alt="Hosting Plan tab on the Azure SWA " />
...
2. Click on the Configuration link within the navigation pane, choose the stage environment and add two application secrets, namely AUTH0_CLIENT_ID, & AUTH0_CLIENT_SECRET, with the values from step 1 of the Auth0 subsection above.
   <Image src={AzureswaApplicationSecretsPortal} alt="Configuration settings on the Azure SWA blade of the Azure Portal" />
...
<Image src={SwacliEmulatorCustomauth} alt="Azure SWA emulator with the provider and username inputs highlighted in red" />
...
<Image src={WeatherforecastCustomauthn} alt="Blazor WASM default page" />
...
<Image src={WeatherforecastCustomauthzFail} alt="Weather Forecast Fetch data page with Not Authorized highlighted on the page" />

### Auth0 Role Management

1. On the Auth0 end, we create a Role named "weathercaster".
    <Image src={Auth0AddRole} alt="Auth0 role management" />

2. Add my username to the role.
   <Image src={Auth0AddUserRole} alt="Auth0 Adding a user to the role" />
...
<Image src={Auth0CustomAction} alt="Auth0 Page with a button named Build Custom alongside another button to Add Action below them a table of existing actions" />

2. Create a custom Action.

<Image src={Auth0AddAction} alt="A modal Auth0 Action Create with 3 input elements" />
...
<Image src={Auth0CustomActionCode} alt="A Monaco code editor is pre-filled with code snippet to add roles to the ID Token and the Access token " />

4. We need to add the roles during our Login Flow.

<Image src={Auth0ChooseFlow} alt="The Flow Page under the Action Tab is highlighted. The different available flows are listed in a card format" />

5. Drag and drop our Custom Action into the Login Flow.

<Image src={Auth0FlowAddAction} alt="A visual canvas with a Start and Complete blobs are present. And the space in between them is highlighted in red by the author" />

6. Apply the changes, and we are done on the Auth0 end.

<Image src={Auth0FlowApplyAction} alt="The Add Visage Role Custom Action is placed between the Start and Complete icons in the canvas. The Apply button has a author-drawn green arrow pointed towards it" />
...
<Image src={Auth0SampleAppProfile} alt="The Auth0 sample app page displaying the payload sent over by Auth0 post login" />
...
<Image src={VscodeAzureSWAAzureFunction} alt="VS Code with highlights of the Command Palette, SWA config file in the code editor tab and thenewly created folder and files under the File Explorer" />
...
<Image src={AzurefunctionApiRoleMgmt} alt="VS Code with the left tab showing the test payload, the right tab displaying the result returned from the Azure Function,and the bottom console displaying the log stream generated by the invoked Azure Function" />
...
<Image src={AzureswaApplInsight} alt="Application Insight tab is highlighted within the Azure Static Web App blade on the Azure Portal" />
...
<Image src={WeatherforecastCustomauthzSuccess} alt="Weather forecast with Auth0 provided login show and side-by-side browser developer console is shown" />
...
<Image src={VscodeAzureSWAExtension} alt="The Azure VS Code extension is expanded out and its various resources are visible" />

But in the quickstart for step 2 [which was removed last month after the team was informed] is shown as below:

<Image src={VscodeAzureSWAExtensionInitial} alt="VS Code extension for Azure Static Web Apps with a plus icon boxed in red highlight and an adjacent lightning icon fenced in green highlight" />
...
<Image src={VscodeAzureSWAMarketplace} alt="Text highlighted in red" />
...
<Image src={MsdocsAzureswaApiConstraints} alt="Text about API route nomenclature highlighted in red" />

But, the VS Code extension generated Github Action workflow file names it as "Api".

<Image src={VscodeExtensionGithubAction} alt="GitHub Action workflow YAML" />

Innocuous, right? Read on.

When we create the API via the command palette as instructed by the MS Docs,

<Image src={VscodeExtensionCommandPalletteCreateApi} alt="VS Code extension command palette expanded with the Create API command in the middle of the list" />

it creates a file folder titled "api" in the SWA folder.

<Image src={VscodeApiFileExplorer} alt="VS Code File Folder side tab with api folder highlighted in red" />

Now,if you test it locally via the SWA CLI using "API"on windows ,it works !!!

<Image src={SwaCliApiOptionWindows} alt="SWA CLI" />

And when pushed to GitHub, no error is thrown and is deployed. But errors out on the browser.

<Image src={WeatherforecastApiNomenclatureFail} alt="Weather browser opened in browser with the developer tool console opened side by side and riddled with http error code" />

If you are diligent enough to go through the successful GitHub Actions logs, there is a hint.

<Image src={GithubActionLogAzureswaApiWarning} alt="GitHub Actions logs with a missing API warning" />
